    ...
    process {
        ...
        try {
            $manifest = [xml](Get-Content $Path)
             ...
             #TYPO
            $ex = New-Object System.Exception ($LocalizedData.ManifestNotWelFormedXml_F2 -f $Path, $_.Exception.Message), $_.Exception
            $category = [System.Management.Automation.ErrorCategory]::InvalidData
            $errRecord = New-Object System.Management.Automation.ErrorRecord $ex,'InvalidManifestFile',$category,$Path
            $psCmdlet.WriteError($errRecord)
            Write-Debug ("test : {0} " - f $LocalizedData.CléInexistanteDansLeFichierDeRessource) 
            return
        }
