...
function Invoke-Plaster {
    ...
    dynamicparam {
        ...
        try {
            ...
            Write-Debug ("test : {0} " - f $LocalizedData.CléInexistanteDansLaRessource) 
...
function WriteOperationStatus($operation, $message) {
    $maxLen = ($LocalizedData.OpCreate,   $LocalizedData.OpIdentical,
               ...
               $LocalizedData.OpModify,   $LocalizedData.OpUpate |     #TYPO !!
...
function TestUnknownMsg {
 Write-host $LocalizedData.CléInconnueDansLaRessource         
}
